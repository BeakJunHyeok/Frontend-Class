// 1. card item 각 요소에 마우스를 오버하면, 각 item들이 위쪽으로 올라오도록 하고싶다.
// 1-1. 웹 브라우저 > card_item들이 어떤 요소들인지 알게끔 도와줘야 한다.
// 1-1-1. 웹 브라우저는 DOM 객체 모델을 갖고 있으니까 해당 DOM에서 card_item들의 Node를 찾아와야 겠다.
// 1-1-1-1. DOM에서 원하는 Node를 찾아오려면 querySelector() 혹은 getElementById()등의 함수를 사용할 수 있다.
// 1-2. card_item이 1개 아니라, 총5개이며,이 5개 모두 동일한 패턴 및 형식의 동적인 이벤트 기능을 가져야한다.
// 2. 만약 마우스가 A item 위에 있다가 B item로 이동을 하게된다면, A는 원래의 위치로 돌아오게하고 B item은 위쪽으로 올라오게 하고 싶다.
...
// 2_2. 하단에 lnb요소의 opactity value 1이 되어야 한다.
// 3. mouseleave ll mouseout 기능을 활용해서 마우스가 옆에 있는 li태그로 이동을 하면 기존에 이벤트는 제거 // 신규이벤트가 이전에 설정했던 기능을 그대로 구현!!
